...
class SeniorValuerValuationListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        # Get valuations for projects assigned to this senior valuer
        # Exclude draft valuations as they are not yet ready for review
        queryset = Valuation.objects.filter(
            project__assigned_senior_valuer=user
        ).exclude(status='draft').select_related('project', 'field_officer').prefetch_related('photos')
...
def senior_valuer_approve_valuation(request, pk):
    ...
    # Only reviewed or submitted valuations can be approved by senior valuer
    if valuation.status not in ['reviewed', 'submitted']:
        return Response(
            {'error': f'Only reviewed or submitted valuations can be approved. Current status: {valuation.status}'},
            status=status.HTTP_400_BAD_REQUEST
        )
...
def senior_valuer_reject_valuation(request, pk):
    ...
    # Only reviewed or submitted valuations can be rejected by senior valuer
    if valuation.status not in ['reviewed', 'submitted']:
        return Response(
            {'error': f'Only reviewed or submitted valuations can be rejected. Current status: {valuation.status}'},
            status=status.HTTP_400_BAD_REQUEST
        )
