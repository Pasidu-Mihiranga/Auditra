...
import { Search, CheckCircle, Cancel, Visibility, CloudUpload } from '@mui/icons-material';
...
export default function ValuationReview() {
  ...
  const fetchValuations = async () => {
    try {
      setLoading(true);
      const res = await valuationService.getReviewed();
  ...
  const handleAction = async (id, action) => {
    try {
      if (action === 'approved') {
        // If there's a file, we need to submit it as a proposal first
        if (reportFile) {
          const formData = new FormData();
          formData.append('final_report', reportFile);
          if (remarks) formData.append('senior_valuer_comments', remarks);

          await valuationService.submitProposal(id, formData);
        } else if (remarks) {
          // If no file but there are remarks, still submit them as comments
          await valuationService.updateValuation(id, { senior_valuer_comments: remarks });
        }

        await valuationService.approveValuation(id);
      } else {
        await valuationService.seniorValuerReject(id, { rejection_reason: remarks });
      }
      setSnackbar({ open: true, message: `Valuation ${action} successfully`, severity: 'success' });
      setRemarks('');
      setReportFile(null);
      setDetailDialog({ open: false, valuation: null });
      fetchValuations();
    } catch (err) {
      setSnackbar({ open: true, message: `Failed to ${action} valuation`, severity: 'error' });
    }
  };
  ...
  const filteredValuations = valuations.filter(v => {
    const statusMatch = tabValue === 0 ||
      ...
      (statusFilters[tabValue] === 'pending' && (v.status === 'submitted' || v.status === 'reviewed'));
    ...
    <Box>
      ...
      <Paper sx={{ mb: 3 }}>
        <Tabs value={tabValue} onChange={(_, v) => setTabValue(v)} sx={{ borderBottom: 1, borderColor: 'divider' }}>
          ...
          <Tab label={`Pending (${valuations.filter(v => ['pending', 'submitted', 'reviewed'].includes(v.status)).length})`} />
      ...
      <Dialog open={detailDialog.open} onClose={() => setDetailDialog({ open: false, valuation: null })} maxWidth="md" fullWidth>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              {(detailDialog.valuation.status === 'pending' || detailDialog.valuation.status === 'submitted') && (
                <>
                  ...
                  />
                  <Box sx={{ mt: 2 }}>
                    <Typography variant="subtitle2" color="text.secondary" gutterBottom>Final Valuation Report (PDF)</Typography>
                    <Button
                      component="label"
                      variant="outlined"
                    ...
                    >
                      {reportFile ? reportFile.name : 'Select Final Report PDF'}
                      <input
                        type="file"
                        hidden
                    ...
                    {reportFile && (
                      <Typography variant="caption" color="success.main" sx={{ mt: 0.5, display: 'block' }}>
                        Ready to upload: {reportFile.name}
                      </Typography>
                    )}
                  </Box>
                </>
        ...
        <DialogActions>
          {['pending', 'submitted', 'reviewed'].includes(detailDialog.valuation?.status) && (
            <>
